...
case "$cmd" in
  ...
  doc*) exec terraform-docs asciidoc document --output-file=README.adoc \
    --output-template="$(cat .README.template.adoc)" \
    "$@" . ;;
  # TFVars Examples are generated by terraform-docs.
  tfvars) exec terraform-docs tfvars hcl --output-file=example-tfvars.hcl \
